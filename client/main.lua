...
local DzSV = Tunnel.getInterface("lc_coguheist") 
		...
		if distance < cfg.computer.draw.dist then
			...
			if IsControlJustPressed(0, cfg.computer.keybind) then
				if distance < 2.0 then
					local cops = DzSV.checkPoliceCount()
					if cops >= cfg.police.minCops then
						local cooldown = DzSV.GetCooldownTimer()
						if cooldown == nil then
							local hasMoney = DzSV.getJobFees()
							if hasMoney then
								OpenHackFunction()
							else
								ShowNotify(Lang['not_enough_money'])
							end
						else
							ShowNotify((Lang['cooldown_time_left']:format(cooldown)))
						end
					else
						ShowNotify(Lang['not_enough_police'])
					end
				else
					ShowNotify('...')
				end
...
RegisterNetEvent("AdicionarCDS:truck")
AddEventHandler("AdicionarCDS:truck",function(x,y,z)
...
function HackCallback(success)
	...
	if success then
		TriggerServerEvent('lc_coguheist:startJobSV')
	else
		ShowNotify(Lang['hacking_failed'])
	end
end
...
function CreateTruckRobberyMapBlip()
	...
	if cfg.blip.enable then
		local mk = cfg.blip
		local cords = {x = cfg.pos[1],y = cfg.pos[2],z = cfg.pos[3]}
		TriggerServerEvent('policeAlert',cords)
...
RegisterNetEvent('lc_coguheist:startJobCL')
AddEventHandler('lc_coguheist:startJobCL',function()
    ...
    if takenNum == 100 then
        ShowNotify(Lang['no_available_jobs'])
    else
        TriggerEvent('lc_coguheist:truckRobberyJob', num)
        PlaySoundFrontend(-1, "Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
		ShowNotify('Vá até o carro forte.')
    end
end)
...
RegisterNetEvent('lc_coguheist:truckRobberyJob')
AddEventHandler('lc_coguheist:truckRobberyJob',function(num)
	local job = Config.TruckSpawns[num]
	Config.TruckSpawns[num].inUse = true
	TriggerServerEvent('lc_coguheist:SyncDataSV', Config.TruckSpawns)
		...
		if job.inUse then 
			...
			if distance < 150.0 then 
				...
				if distance < 100.0 and not ArmoredTruckSpawned then
					...
					ArmoredTruck = CreateArmoredTruck('bison', job.pos)
				...
				if ArmoredTruckSpawned and SecuritySpawned then 
					...
					if truck_dist <= 5.0 and not TruckDemolished then
						...
						if GetEntityModel(closest_veh) == GetHashKey('bison') then
...
function BlowTheTruckDoor()
	...
	if IsVehicleStopped(ArmoredTruck) then
		...
		ShowNotify(Lang['begin_to_rob'])
	else
		ShowNotify(Lang['truck_not_stopped'])
	end
end
...
function RobbingTheMoney()
	...
	TriggerServerEvent('lc_coguheist:jobReward')
	Citizen.Wait(1000)
	StopTheJob = true
end
...
-- Sync Config Data:
RegisterNetEvent('lc_coguheist:SyncDataCL')
AddEventHandler('lc_coguheist:SyncDataCL',function(data)
